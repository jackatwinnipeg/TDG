...
function computeRemainingTDG(baseTdgKg, records) {
  ...
  const totalReloadKg = list.reduce(
    (sum, record) => sum + getReloadAmountKg(record),
    0,
  );
  ...
  const totalDeliveredLbs = list.reduce(
    (sum, record) =>
      sum +
      (Number(record?.deliveredVolume ?? record?.delivered_volume) || 0),
    0,
  );
...
function openReloadModal() {
  ...
  setReloadError("");
  if ($("reloadAmountKg")) $("reloadAmountKg").value = "";
  modal.hidden = false;
  modal.setAttribute("aria-hidden", "false");
  document.body.style.overflow = "hidden";

  requestAnimationFrame(() => $("reloadAmountKg")?.focus());
}
...
async function getReloadLocationCustomer() {
  const local = getCustomers().find(
    (customer) =>
      String(customer?.accountNumber || "").trim() === RELOAD_ACCOUNT_NUMBER,
  );
  ...
  if (!sb?.from) {
    return {
      accountNumber: RELOAD_ACCOUNT_NUMBER,
      accountName: RELOAD_DEFAULT_NAME,
      accountAddress: "",
      city: "",
      route: "",
    };
  ...
  const { data, error } = await sb
    .from("tdg_customers")
    .select("account_number, account_name, account_address, city, route")
    .eq("account_number", RELOAD_ACCOUNT_NUMBER)
    .maybeSingle();

  if (error) throw new Error(`读取 Reload Location 失败：${error.message}`);

  return {
    accountNumber: data?.account_number || RELOAD_ACCOUNT_NUMBER,
    accountName: data?.account_name || RELOAD_DEFAULT_NAME,
...
async function saveReloadEvent() {
  ...
  const amountKg = integerPart($("reloadAmountKg")?.value);

  setReloadError("");

  if (!(amountKg > 0)) {
    setReloadError("Reload Amount 必须是大于 0 的整数 kg。");
    $("reloadAmountKg")?.focus();
    return;
  }

  const sess = getAuthSessionSafe();
  if (!sess?.userId) {
    setReloadError("登录状态已失效，请重新登录。");
    return;
  }
  ...
  try {
    const location = await getReloadLocationCustomer();
    const completedAt = tdgLocalDateTimeISO();

    const event = {
      eventType: "reload",
      reloadAmountKg: amountKg,
      ...
      arrivalTime: nowHHMM(),
      completedAt,
      accountNumber: location.accountNumber || RELOAD_ACCOUNT_NUMBER,
      accountName: location.accountName || RELOAD_DEFAULT_NAME,
      accountAddress: location.accountAddress || "",
      accountCity: location.city || "",
      accountRoute: location.route || "",
      tdgVolume: baseTdgKg,
      deliveredVolume: 0,
      notes: `TDG Reload: +${amountKg} kg`,
      done: true,
    ...
    console.error("Save reload event failed:", error);
    setReloadError(error?.message || "Reload 保存失败，请检查网络后重试。");
  } finally {
    window.__savingReload = false;
    if (button) {
...
function getPreviousCalendarDate(sourceDate = new Date()) {
...
async function importPreviousDayTdgBalanceOncePerDay({ force = false } = {}) {
  const sess = window.TDG_AUTH?.getSession?.();
  if (!sess?.userId) {
  ...
  try {
    ...
    if (!balance.found || balance.remainingKg == null) {
      ...
      return {
        ok: true,
        imported: false,
        reason: "no_valid_previous_day_data",
        balance,
      };
    }
    ...
    if (
      ...
      if (imported) {
        toast(
          "已从缓存导入余量",
          `云端暂不可用，已导入 ${integerPart(cached.remainingKg)} kg。`,
        );
      }
    ...
    return {
      ok: false,
      reason: "supabase_error_no_valid_cache",
...
window.TDG_SYNC = {
  ...
  forceRefreshPreviousDayTdgBalance() {
    return importPreviousDayTdgBalanceOncePerDay({ force: true });
  },
};
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const fillDriverFromSession = (session) => {
    const driverNumber =
  ...
  try {
    ...
    console.log(
      "Previous-day TDG import result:",
      importResult,
    );
  } catch (error) {
    console.warn(
  ...
  $("btnSearch")?.addEventListener(
    ...
    () => {
      ...
      renderResults(searchCustomers(query));
    },
  );
  ...
  $("btnReload")?.addEventListener("click", openReloadModal);
  $("btnReloadClose")?.addEventListener("click", closeReloadModal);
  ...
  $("reloadAmountKg")?.addEventListener(
    "keydown",
    (event) => {
      if (event.key === "Enter") {
        event.preventDefault();
  ...
  $("btnLoadProfile")?.addEventListener(
    "click",
    async () => {
  ...
  $("weekCycle")?.addEventListener(
    "change",
    async () => {
      const value = Number($("weekCycle")?.value || 1);
      ...
      try {
        ...
        toast(
          "已更新",
          `Week Cycle 已设为 ${value}，将从本周起按周自动递增。`,
        );
      } catch (error) {
        console.warn(
  ...
  console.log(
    "TDG page initialization completed.",
    {
      restored,
      draftRestored,
      tdgVolume:
