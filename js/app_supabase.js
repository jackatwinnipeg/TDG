...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    console.warn("Previous-day TDG auto import failed:", error);
  }
  ...
  $("reloadModal")?.addEventListener("click", (event) => {
    if (event.target === $("reloadModal")) closeReloadModal();
  });
  $("reloadAmountKg")?.addEventListener("keydown", (event) => {
    if (event.key === "Enter") saveReloadEvent();
  });
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !$("reloadModal")?.hidden) {
      closeReloadModal();
    }
  });
