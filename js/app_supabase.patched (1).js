// ---------------------------
      ...
      async function syncCustomersFromServer({ silent = true } = {}) {
        ...
        if (!sb?.from) {
          if (!silent) toast("未就绪", "Supabase client 尚未初始化。");
        ...
        try {
          ...
          if (error) {
            if (!silent) toast("同步失败", error.message || "无法读取客户库。");
          ...
          const list = (data || []).map((row) => ({
            ...
            accountAddress: String(row.account_address ?? row.accountAddress ?? ""),
            city: String(row.city ?? ""),
            route: String(row.route ?? row.route_type ?? row.routeType ?? ""),
            routeType: String(row.route_type ?? row.routeType ?? row.route ?? ""),
          ...
          localStorage.setItem(LS_CUSTOMERS, JSON.stringify(list));
          if (!silent) toast("已同步", `已从 Supabase 同步 ${list.length} 条客户。`);
          return { ok: true, count: list.length, items: list };
        } catch (e) {
          if (!silent) toast("同步失败", "网络异常，已继续使用本地客户库。");
          return { ok: false, reason: "network", error: e };
        }
      }

      const LS_HISTORY_CACHE = "tdg_history_cache_v1";

      async function pullHistoryRecordsFromSupabase({ silent = true } = {}) {
        const sb = window.supabaseClient;
        if (!sb?.from) {
          if (!silent) toast("未就绪", "Supabase client 尚未初始化。");
          return { ok: false, reason: "no_supabase" };
        ...
        try {
          ...
          if (error) {
            if (!silent) toast("历史同步失败", error.message || "无法读取历史记录。");
            return { ok: false, reason: "query_error", error };
          }
          ...
          localStorage.setItem(LS_HISTORY_CACHE, JSON.stringify(list));
          if (!silent) toast("历史已同步", `已从 Supabase 同步 ${list.length} 条历史记录。`);
          return { ok: true, count: list.length, items: list };
        } catch (e) {
          if (!silent) toast("历史同步失败", "网络异常，已继续使用本地历史缓存。");
          return { ok: false, reason: "network", error: e };
        }
      }
...
// Auto-fill TDG Volume from yesterday remaining (per-driver)
  ...
  // Auto-sync customers + history from Supabase
  ensureDailyCloudSyncAfterLogin({ silent: true }).catch((e) => console.warn("Daily cloud sync failed:", e));
        ...
        // Auto-sync customers + history from Supabase (best-effort)
        // If offline / not logged in, it will silently keep using local caches.
        ensureDailyCloudSyncAfterLogin({ silent: true }).catch((e) => console.warn("Daily cloud sync failed:", e));
      });
...
/* 每日首次登录同步：
   1) 客户库从 Supabase 拉取
   2) 全部历史记录从 Supabase 拉取
   3) 当天本人的记录从 Supabase 拉取并覆盖本地缓存 */
async function ensureDailyCloudSyncAfterLogin({ force = false, silent = true } = {}) {
  const sess = window.TDG_AUTH?.getSession?.();
  if (!sess?.driverNumber) return { ok: false, reason: "no_session" };

  const today = tdgToday();
  const key = tdgPullKey(sess.driverNumber) + "__cloud_sync";
  const last = localStorage.getItem(key);

  if (!force && last === today) {
    console.log("TDG daily cloud sync already done");
  ...
  try {
    result.customers = await syncCustomersFromServer({ silent });
    if (result.customers?.ok === false) result.ok = false;
  } catch (e) {
    result.ok = false;
    result.customers = { ok: false, reason: "exception", error: e };
  }

  try {
    result.history = await pullHistoryRecordsFromSupabase({ silent });
    if (result.history?.ok === false) result.ok = false;
  } catch (e) {
    result.ok = false;
    result.history = { ok: false, reason: "exception", error: e };
  }

  try {
    result.today = await pullTodayRecordsFromSupabase();
  } catch (e) {
    result.ok = false;
    result.today = { ok: false, reason: "exception", error: e };
  }

  localStorage.setItem(key, today);
  console.log("TDG daily cloud sync finished:", result);
  return result;
}

const ensureDailyPullAfterLogin = ensureDailyCloudSyncAfterLogin;

window.TDG_SYNC = {
  ensureDailyPullAfterLogin,
  ensureDailyCloudSyncAfterLogin,
  pullTodayRecordsFromSupabase,
  pullHistoryRecordsFromSupabase
};
